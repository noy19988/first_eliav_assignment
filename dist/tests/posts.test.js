...
describe('Posts API Tests', () => {
    ...
    it('should save a post', async () => {
        ...
        const saveResponse = await (0, supertest_1.default)(app)
    ...
    it('should get posts by user', async () => {
        const response = await (0, supertest_1.default)(app).get(`/posts/user/${userId}`);
        expect(response.status).toBe(200);
        expect(response.body.length).toBeGreaterThan(0);
    });
    ...
    it('should not get posts by user with invalid user ID', async () => {
        const response = await (0, supertest_1.default)(app).get('/posts/user/invalid-id');
        expect(response.status).toBe(400);
    });
    ...
    it('should handle post update with liked/unliked', async () => {
        const createResponse = await (0, supertest_1.default)(app)
            ...
            .attach('image', path_1.default.join(__dirname, 'test-image.png'));
        expect(createResponse.status).toBe(201);
        const updateLikedResponse = await (0, supertest_1.default)(app)
            .put(`/posts/${createResponse.body.post._id}`)
            .set('Authorization', `Bearer ${token}`)
            .send({ liked: true });
        expect(updateLikedResponse.status).toBe(200);
        expect(updateLikedResponse.body.post.likes).toBe(1);
        const updateUnlikedResponse = await (0, supertest_1.default)(app)
            .put(`/posts/${createResponse.body.post._id}`)
            .set('Authorization', `Bearer ${token}`)
            .send({ liked: false });
    ...
    it('should handle post update with saved/unsaved', async () => {
        const createResponse = await (0, supertest_1.default)(app)
            ...
            .attach('image', path_1.default.join(__dirname, 'test-image.png'));
        expect(createResponse.status).toBe(201);
        const updateSavedResponse = await (0, supertest_1.default)(app)
            .put(`/posts/${createResponse.body.post._id}`)
            .set('Authorization', `Bearer ${token}`)
            .send({ saved: true });
        expect(updateSavedResponse.status).toBe(200);
        expect(updateSavedResponse.body.post.savedBy).toContain(userId);
        const updateUnsavedResponse = await (0, supertest_1.default)(app)
            .put(`/posts/${createResponse.body.post._id}`)
            .set('Authorization', `Bearer ${token}`)
            .send({ saved: false });
    ...
    it('should delete a post', async () => {
        const response = await (0, supertest_1.default)(app)
            ...
            .set('Authorization', `Bearer ${token}`);
        expect(response.status).toBe(200);
        const getResponse = await (0, supertest_1.default)(app).get(`/posts/${postId}`);
